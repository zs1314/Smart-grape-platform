...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):  # path:data//images//bus/jpg，img_size:[640,640]
        p = str(Path(path).resolve())  # os-agnostic absolute path,根据相对路径（path）得到绝对路径（D:\yolov5-6.1\data\images\bus.jpg）
        if '*' in p:
            # 判断是否带星号
    ...
    def __next__(self):
        ...
        # Padded resize
        # 把原图变成特定大小的图片（[640,480,3],HWC）——按长边进行缩放，方便后面进行预测
        ...
        # 图片路径、resize后的图片、原图、none、一个要打印的字符串的信息
